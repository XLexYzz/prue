...
//		numPartidas++;
	...
	private void visualizarSalida(boolean acertada, int i) {
		System.out.println ();
		if (acertada) {
			quebien = "�ENHORABUENA! Has acertado la combinaci�n en ";
			System.out.println (quebien + (i-1) +" intentos en la partida n� " + numPartidas);
		} else {
			quepena = "LO SENTIMOS. No has conseguido acertar la combinaci�n en la partida n� ";
	...
	private boolean comprobarJugada (int j) {
		int i,k;
	// Las siguientes 8 variables llevar�n el n�mero de muertos de cada color
		int numRM = 0, numVM = 0, numAM = 0, numBM = 0;
		int numNM = 0, numMM = 0, numGM = 0, numSM = 0;
	// Las siguientes 8 variables tendr�n el n�mero de piezas del mismo color
	// que hay en la combinaci�n a adivinar
		int numRComb = 0, numVComb = 0, numAComb = 0, numBComb = 0;
		int numNComb = 0, numMComb = 0, numGComb = 0, numSComb = 0;
	// Las siguientes 8 variables tendr�n el n�mero de piezas del mismo color
	// que hay en la jugada que se est� comprobando
		...
		heridos = extracted(numRM, numVM, numAM, numBM, numNM, numMM, numGM, numSM, numRComb, numVComb, numAComb,
				numBComb, numNComb, numMComb, numGComb, numSComb, numRJugada, numVJugada, numAJugada, numBJugada,
				numNJugada, numMJugada, numGJugada, numSJugada, heridos);
	...
	private int extracted(int numRM, int numVM, int numAM, int numBM, int numNM, int numMM, int numGM, int numSM,
